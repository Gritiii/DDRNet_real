...
drop_path = 0.1  # Example drop path rate
attn_drop_rate = 0.1  # Example attention dropout rate
d_state = 16  # Example d_state value

# Instantiate VSSBlock
vss_block = VSSBlock(hidden_dim=hidden_dim, drop_path=drop_path, attn_drop_rate=attn_drop_rate, d_state=d_state)

# Generate a random input tensor
input_tensor = torch.randn(1,16, 16, hidden_dim)  # Example shape: (batch_size=1, hidden_dim, height=16, width=16)
device = 'cuda' if torch.cuda.is_available() else 'cpu'
input_tensor = input_tensor.to(device)
# Forward pass
output_tensor = vss_block(input_tensor)
